...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        for (int i = 0; i < answers.length; i++) {// herbir ogrenciye bakar.
            ...
            for (int j = 0; j < answers[i].length; j++) {// herbir ogrenci cevaplarına bakar
                if(answers[i][j] == keys[j]){// ogrenci cevap ile cevap anahtari karsilastirir
                   sayac++;
                }
            }
